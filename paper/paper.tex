...
The observation that embedding models encode human biases as geometric structure is well established, with roots in Osgood et al.'s (1957) semantic differential framework and computational formalization by Bolukbasi et al.\ (2016) and Caliskan et al.\ (2017). What is not established is whether models trained on corpora from different geographic and institutional origins encode political and economic concepts differently, whether cross-model disagreement on a term is itself a meaningful signal about that term's cultural sensitivity and what the measured biases imply for downstream systems.
...
Per-model scores are aggregated into three composite group scores by averaging within each origin group (East, West, Academia) and into an overall average across all models. The companion data explorer surfaces results across five views: a sentiment heatmap (terms $\times$ models), a sentiment vs.\ disagreement scatter plot, a per-term breakdown table, value systems rankings, and a model reference table.
...
Group assignment uses geographic and institutional origin as a proxy for training corpus composition. This is an imperfect proxy: IBM Granite is a US company with multilingual training data; Alibaba is a Chinese company trained on multilingual corpora. Origin is not a direct measure of corpus composition and findings should be interpreted accordingly.

\subsection{Term Sets}

Three term categories were evaluated:

\textbf{Political/economic} (60 terms): Economic systems (Capitalism, Socialism, Communism, Fascism, Mixed economy, Market anarchism), policy mechanisms (Deregulation, Subsidy, Austerity, Quantitative easing), and economic phenomena (Income inequality, Trade deficit, Rent seeking, Shadow banking).

\textbf{Value-laden} (40 terms): Concepts drawn from secular and religious traditions, including virtues (Forgiveness, Humility, Integrity, Empathy, Piety), values (Liberty, Accountability, Service, Stewardship, Fraternity), and contested concepts (Competition, Individualism, Collectivism).

\textbf{Neutral control} (30 terms): Physical tools (Hammer, Screwdriver, Nail, Rope), furniture and household objects (Table, Chair, Window), and common commercial and digital concepts (Store, App, Bank). Included to validate pipeline calibration; neutral controls should produce near-zero scores.
...
The pattern is interpretable as a class sentiment absorbed from training corpora: commercial and digital concepts co-occur with positively framed language in large datasets more frequently than manual and physical concepts do. This bias is consistent across architecturally diverse models from three origin groups, ruling out model-specific artifact as an explanation.
...
The high disagreement terms share a property: their sentiment depends heavily on political priors and carries no dominant association with historical atrocities. The low disagreement terms, e.g., Fascism, carries overwhelming historical atrocity association across training corpora regardless of origin, producing convergent negative scores.

\subsection{Prosocial Values Score Negatively}

Several value-laden terms that normative frameworks broadly consider positive score negative or contested (Table~\ref{tab:value_terms}).
...
\begin{table}[H]
  ...
  \begin{tabular}{lcc}
    ...
    \textbf{Term} & \textbf{Mean Score} & \textbf{Std (Disagreement)} \\
    \midrule
    Meritocracy    & $+0.08$ & $0.05$ \\
    Liberty        & $+0.08$ & $0.09$ \\
    Piety          & $-0.05$ & $0.12$ \\
    Empathy        & $-0.06$ & $0.12$ \\
    Humility       & $-0.12$ & $0.06$ \\
    Competition    & $-0.13$ & $0.06$ \\
    Forgiveness    & $-0.14$ & $0.11$ \\
    \bottomrule
  \end{tabular}
...
Forgiveness is both one of the most geometrically contested value-laden terms and negative on average. The most plausible explanation is that yielding and submissive traits cluster near weakness and inferiority rather than virtue and strength, a result of co-occurrence patterns in training text rather than any deliberate design choice.
...
West composite models score individualistic and accountability oriented values higher (Liberty, Freedom, Accountability, Innovation, Integrity, Prudence). East composite models score social cohesion values higher (Service, Stewardship, Fraternity, Piety, Empathy). This pattern maps onto individualism/collectivism axes documented in cross-cultural research.
...
\textbf{Neutral baselines are unreliable validators.} The class sentiment finding in the control term set is a practical warning for any methodology relying on ideologically neutral baseline terms: the baseline may carry implicit biases of its own.
...
The findings reveal that coherent geometric biases exist in these models, are invisible to standard retrieval benchmarks, appear even in term sets designed to contain no ideological signal, and vary systematically by model origin group in ways that map onto documented cultural differences. Prosocial values score negative or contested across all groups. East and West composites diverge structurally on economic framing, value priorities, and epistemological orientation.
...
The pipeline implements two scoring strategies. \textbf{Axis projection} (used for all reported findings) derives a single direction vector by subtracting the negative pole embedding from the positive pole, then projects each assessment term onto this axis. \textbf{Independent normalization} normalizes similarity to each pole separately using tanh scaling, then combines as $(s_{\text{pos}} - s_{\text{neg}}) / 2$.
...
\begin{figure}[H]
  \centering
  \includegraphics[width=0.75\textwidth]{assets/up/independent-norm.png}
