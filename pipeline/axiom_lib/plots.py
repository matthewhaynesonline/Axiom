...
def _plot_heatmap(
    ...
    values_abs = values_abs if values_abs > 0 else 1.0
    ...
    for i in range(len(terms)):
        for j in range(len(model_ids)):
            val = matrix[i, j]
            # Use white text on dark backgrounds (extremes) and black on light backgrounds (middle)
            text_color = "white" if abs(val) > (values_abs * 0.5) else "black"
            ax.text(
                j,
                i,
                f"{val:.2f}",  # Formats the number to 2 decimal places
                ha="center",
                va="center",
                color=text_color,
                fontsize=8,
