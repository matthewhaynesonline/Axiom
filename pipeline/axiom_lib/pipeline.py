...
from .normalization import zscore, zscore_np, minmax, tanh_scale_np
...
def process_data(
    ...
    a_category: str,
    a_category_data: dict,
    b_category: str,
    b_category_data: dict,
    out_dir: str,
    ...
    scores = model.similarity(
        a_category_data["embeddings"], b_category_data["embeddings"]
    )

    for (
        a_term_index,
        a_term,
    ) in enumerate(a_category_data["terms"]):
        for b_term_index, b_term in enumerate(b_category_data["terms"]):
            score = scores[a_term_index][b_term_index].item()
...
def get_avg_sentiment_independent_normalization(
    ...
    Scores are in the open interval (-1.0, 1.0) — never exactly ±1.0
    ...
    # Both outputs are already centred on 0, so subtract the negative from the positive
    avg_vals = positive_normalized - negative_normalized
...
def get_avg_sentiment_axis_projection(
    ...
    projections_norm = projections

    if should_zscore:
        projections_norm = zscore_np(projections_norm)

    if should_tanh:
        projections_norm = tanh_scale_np(projections_norm)
